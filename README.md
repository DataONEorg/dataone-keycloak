...
| `global.orcid.clientId`      | ORCID client identifier from orcid.org               | `PUT-your-orcid-clientid-here`       |
| `global.orcid.baseUrl`       | Endpoint URL for base ORCID auth services            | `https://sandbox.orcid.org/oauth`    |
| `global.orcid.userInfoUrl`   | Endpoint URL for accessing user info from ORCID      | `https://api.sandbox.orcid.org/v3.0` |
| `global.client.rootUrl`      | client root URLfor default OIDC client to be set up  | `https://api.test.dataone.org`       |
| `global.client.clientSecret` | client secret for default OIDC client to be set up   | `PUT-your-oidc-client-secret-here`   |
| `global.realm.frontend`      | URL of the frontend for the deployed realm           | `https://auth.test.dataone.org`      |
...
| `keycloakx.extraInitContainers`                                                     | Additional init containers, e. g. for providing custom themes and providers      | ```- name: keycloak-init  image: ghcr.io/dataoneorg/keycloak-init:0.2.0```                                                                                                                                                        |
| `keycloakx.command`                                                                 | Overrides the default entrypoint of the Keycloak container                       | `["sh"]`  |
| `keycloakx.args`                                                                    | Command args override the default entrypoint of the Keycloak container           | `["-c","/opt/keycloak/bin/kc.sh build --vault=file && /opt/keycloak/bin/kc.sh --verbose start --http-port=8080 --hostname https://auth.test.dataone.org --hostname-strict=false --spi-events-listener-jboss-logging-success-level=info --spi-events-listener-jboss-logging-error-level=warn --spi-theme--static-max-age=-1 --spi-theme--cache-themes=false --spi-theme--cache-templates=false --log-level=error --log-level-org.keycloak.events=debug --log-level-org.keycloak.social.user_profile_dump=debug --log-level-org.keycloak.vault=trace --log-level-org.keycloak.services=info --vault-dir=/opt/keycloak/vault --import-realm --optimized"]` |
| `keycloakx.extraEnv`                                                                | Additional environment variables for Keycloak                                    | `- name: KEYCLOAK_ADMIN  value: kcadmin`  |
| `keycloakx.extraVolumes`                                                            | Add additional volumes, e. g. for custom themes and providers                    | `- name: providers. emptyDir: {}`    |
| `keycloakx.extraVolumeMounts`                                                       | Add additional volumes mounts, e. g. for custom themes and providers             | `- name: providers  mountPath: /opt/keycloak/providers` |
...
| `keycloakx.ingress.annotations.nginx.ingress.kubernetes.io/proxy-buffer-size`       | Default proxy buffer size                                                        | `256k`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `keycloakx.database.size`                                                           | The size to be used by CloudNativePG to create storage volumes.                  | `100Gi`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `keycloakx.http.relativePath`                                                       | The path at which the ingress will respond, typically `/auth`                    | `/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
